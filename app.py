...
# Needed so the session cookie still works when this app is loaded
# inside an iframe on someone else's website (a different domain).
...
# ---------------------------------------------------------------
# 1) DEMO PAGE - a fake homepage, just for showing clients the look.
#    This is NOT what gets embedded on a real site.
# ---------------------------------------------------------------
DEMO_PAGE = """
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        body { font-family: 'Helvetica Neue', Arial, sans-serif; margin: 0; background: #f0ede4; }
...
<body>
    ...
    <div class="site-body">
        <h2>Welcome</h2>
        <p>This is a stand-in for the real Green Mango website. The chat
        widget in the bottom right is loaded the exact same way it would
        be on the real site - via one script tag.</p>
    </div>
...
# ---------------------------------------------------------------
# 2) WIDGET.JS - the one line a real client adds to their site.
#    It injects a small floating button, and an iframe that only
#    appears when clicked. The iframe keeps all our styling totally
#    separate from the client's own site, so nothing clashes.
# ---------------------------------------------------------------
WIDGET_JS = """
(function () {
    ...
    bubble.style.cssText = 'position:fixed;bottom:24px;right:24px;width:60px;height:60px;border-radius:50%;background:#2f4a3c;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,0.25);z-index:999999;transition:transform 0.15s ease;';
    bubble.onmouseenter = function () { bubble.style.transform = 'scale(1.06)'; };
    bubble.onmouseleave = function () { bubble.style.transform = 'scale(1)'; };

    var iframe = document.createElement('iframe');
    iframe.id = 'gm-chat-iframe';
...
# ---------------------------------------------------------------
# 3) WIDGET-FRAME - just the chat UI itself, loaded inside the
#    iframe. No fake homepage here, just the chat window.
# ---------------------------------------------------------------
WIDGET_FRAME = """
<!DOCTYPE html>
<html>
<head>
    <style>
        * { box-sizing: border-box; }
        ...
        #chatHeader { background: #2f4a3c; color: white; padding: 16px 18px; }
        #chatHeader .title { font-size: 15px; font-weight: 600; }
        #chatHeader .subtitle { font-size: 12px; opacity: 0.75; }
        #chatbox { flex: 1; padding: 16px; overflow-y: auto; background: #faf9f5; }
        ...
        #sendBtn { border: none; background: #2f4a3c; color: white; width: 38px; height: 38px; border-radius: 50%; margin-left: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
